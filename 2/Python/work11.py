class MooreMachine:
    ...
    def _try_transition(self, action):
        print(f'Trying action {action} from state {self.state}')
        if action in self.transitions.get(self.state, {}):
            new_state, output = self.transitions[self.state][action]
            print(f'Transition to {new_state} with output {output}')
            self.state = new_state
            self.output = output
            self.visited_states.add(new_state)
            return None
        else:
            print(f'Action {action} unsupported from state {self.state}')
            return 'unsupported'
